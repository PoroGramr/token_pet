...
final class CharacterEditorWindowController: NSObject, NSWindowDelegate {
    ...
    init(
        ...
        let model = CharacterManagerModel(store: store, repository: repository, onApply: onApply)
        self.model = model

        let hostingController = NSHostingController(rootView: CharacterManagerView(model: model))
        let window = NSWindow(
    ...
    private func confirmDiscard() -> Bool {
        ...
        alert.messageText = "저장하지 않은 변경을 버릴까요?"
        alert.informativeText = "편집한 이미지와 설정은 저장되지 않습니다."
